...
func run() -> void:
	...
	creature.carry.drop()
	# Player-facing acquisition: look at the visible corpse volume rather than at
	# a physical bone coordinate known only to the test.
	creature.carry.camera.global_position = corpse.global_position + Vector3(0.0, 1.0, 2.4)
	creature.carry.camera.look_at(corpse.global_position + Vector3.UP * 0.9, Vector3.UP)
	creature.carry.begin_grab(creature)
	assert(creature.carry.carried != null, "Looking at the visible corpse did not acquire any ragdoll body")
	creature.carry.drop()
	Input.action_press(&"interact")
	creature._process_action_input()
	assert(creature.carry.carried != null, "Pressed interact action did not begin corpse dragging")
	Input.action_release(&"interact")
	creature._process_action_input()
	assert(creature.carry.carried == null, "Released interact action did not drop the corpse")
