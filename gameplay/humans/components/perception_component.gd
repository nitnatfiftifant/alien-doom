...
func evaluate_target(target: Node3D, delta: float, is_corpse := false) -> void:
	if target == null:
		return
...
func _has_line_of_sight(target_position: Vector3, expected: Node3D = null) -> bool:
	...
	return result.is_empty() or (expected != null and result.collider == expected)
...
func _on_noise(position: Vector3, loudness: float, source: Node) -> void:
	...
	var strength := loudness * occlusion * (1.0 - distance / maxf(hearing_distance, 0.01)) * 30.0
